...
public class RequestParamTest {
	...
	@ExceptionHandler(Exception.class)	// @ExceptionHandler()가 있을 경우 () 안에 있는 예외가 발생시 해당 메소드가 실행됨
...
//		public String main3(@RequestParam(name="year", required=true) String year) {   // 아래와 동일 
		public String main3(@RequestParam String year) {	// @RequestParam을 사용시 required는 기본값이 true   
...
//		http://localhost/ch2/requestParam7?year   ---->> 400 Bad Request, nested exception is java.lang.NumberFormatException: For input string: "" 
	...
	@RequestMapping("/requestParam8")  // required=false(필수입력 X)이기 때문에 밑의 줄과 같은 경우 year=null 이기 때문에 int로 변환 불가. 서버 에러. 기본값 설정이 필요
	public String main8(@RequestParam(required=false, defaultValue="2022") int year) {  // defaultValue="2022" -> 사용자가 입력한 값은 null이지만 controller가 받은 값은 "2022"
